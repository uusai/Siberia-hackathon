...
#
# request_log (миграция 023) — журнал обращений к самому ассистенту, без
# имени спрашивавшего и без текста SQL. Закрывает требование памятки
# «возможность аналитики пользовательских запросов через ИИ»: до него
# статистику отдавал только готовый отчёт /meta/stats, а спросить своими
# словами — «сколько запросов отклонено за сутки», «на чём чаще всего не
# находится данных» — было нельзя.
_ANALYTICS_ADMIN = {"applications_by_day", "admission_dynamics", "request_log"}
