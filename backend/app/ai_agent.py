...
# Обвязка внешних источников: откуда строка взялась и когда сверялась.
#
# Модели она не нужна НИ ДЛЯ ЧЕГО. Ассистент отвечает по базе данных, а не по
# сайту, с которого её однажды наполнили: пользователю нужны цифры, а не
# адрес PDF-файла. При этом в схеме эти колонки занимали 101 позицию из 641 —
# каждая шестая строка описания уходила на то, что запрещено выбирать.
#
# Раньше запрет держался правилом в промпте («не добавляй data_status и
# source_url в SELECT»). Правило приходилось писать, потому что колонки были
# видны; теперь их не видно, и правило удалено — вместе с целым пунктом.
# Это тот же приём, что с висячими внешними ключами: чего модель не видит,
# того она не выдумает.
#
# В базе колонки остаются: происхождение данных нужно загрузчикам, отчётам и
# проверкам целостности. Скрыт только взгляд модели.
_PROVENANCE_COLUMNS = {
    "source_url", "source_id", "checked_at", "data_status",
    "page_url", "site_url",
}
...
def build_sql_system_prompt(role: str | None = None) -> str:
    ...
    if "programs_admission" in allowed:
        admission_rule = (
            "\n10. ВОПРОСЫ О ПОСТУПЛЕНИИ. Официальный справочник университета "
            "лежит "
            ...
            "date_to, description FROM admission_deadlines WHERE "
            ...
            "   НЕ КЛАДИ В ПОИСК РОДОВОЕ СЛОВО. «Факультет», «институт», "
            "«кафедра», «направление» есть в КАЖДОМ названии, и запрос "
            "plainto_tsquery('russian', 'факультет IT') превращается в поиск "
            "одного слова «факультет» — он совпадёт со всеми строками, и "
            "фильтр перестанет фильтровать. Ответ при этом получится "
            "неверным молча. Передавай только различающую часть: "
            "'информационных технологий', 'программная инженерия', 'айти'.\n"
            "   Латинские сокращения из двух букв («IT») разбор русского "
            "языка отбрасывает целиком — искать по ним нечего. Если в вопросе "
            "только такое сокращение и подобрать русское название нельзя, "
            "лучше вернуть пустой результат, чем отфильтровать по родовому "
            "слову и выдать чужие строки за ответ.\n"
            "   ПОИСК ПО НАЗВАНИЮ — ТОЛЬКО ЧЕРЕЗ search_vector. У "
            "programs_admission, program_exam_sets и passing_scores_view есть "
            "колонка search_vector с русской морфологией. Передавай в неё "
            "слова пользователя КАК ЕСТЬ, ничего не переводя и не угадывая: "
            "WHERE search_vector @@ plainto_tsquery('russian', 'программной "
            "инженерии'). Падежи она приводит сама.\n"
            "   Этот же вектор ищет ПОДРАЗДЕЛЕНИЕ и понимает разговорные "
            "сокращения: «физмат», «юрфак», «истфак», «имит», «филфак», "
            "«педфак». Так что «какие направления на юрфаке» — это WHERE "
            "search_vector @@ plainto_tsquery('russian', 'юрфак').\n"
            "   НЕ ПИШИ unit_name ILIKE '%...%' и не подставляй вместо "
            "сокращения своё предположение: «юрфак» в каталоге называется "
            "«Юридический ИНСТИТУТ», а не «юридический факультет», и такой "
            "запрос вернёт пусто. ILIKE оставь только для объектов, у которых "
            "search_vector нет вовсе."
        )
    ...
    # Правила про служебные колонки здесь больше нет.
    #
    # Оно требовало «не добавляй data_status, source_url, source_id,
    # checked_at и page_url в SELECT» — целый пункт промпта, посвящённый
    # тому, чего делать нельзя. Правило было нужно ровно потому, что колонки
    # показывались в схеме. Теперь они оттуда убраны
    # (_PROVENANCE_COLUMNS), и запрещать нечего: невидимую колонку не
    # выберешь. Один пункт правил и 101 позиция схемы — минус.
    provenance_rule = ""
    ...
    if "subject_performance" in allowed or "room_availability" in allowed:
        analytics_rule = (
            ...
            "   - рейтинг успеваемости -> student_rankings (group_name, "
            "course, program_name, faculty_name, semester, avg_score, "
            "grades_count). ФИО студентов там НЕТ и получить их неоткуда: "
            "данные об обучающихся выдаются только обезличенно. Если просят "
            "«с именами» — покажи обезличенный рейтинг и скажи, что имена "
            "студентов система не раскрывает;\n"
            ...
            "   - РАБОТА САМОГО АССИСТЕНТА -> request_log (строка = одно "
            "обращение): asked_at, asked_on, asked_by_role, question, "
            "verdict, reject_reason, row_count, duration_ms, llm_ms, db_ms. "
            "Сюда идут вопросы «сколько запросов за сутки», «сколько "
            "отклонено и почему», «как быстро отвечаем», «на чём чаще всего "
            "не находится данных». Исходы: ok — ответ отдан, rejected — "
            "отклонено проверкой, empty — данных не нашлось, out_of_scope — "
            "вопрос вне области, db_error — СУБД не выполнила запрос, "
            "placeholder — модель выдала бланк, no_sql — вопрос не распознан. "
            "«За сутки» — WHERE asked_at > now() - interval '24 hours'.\n"
...
def build_interpret_system_prompt() -> str:
    return (
        ...
        # Служебные пометки в ответ не выносим: ответ должен читаться как
        # ответ, а не как справка о состоянии базы. Сами колонки происхождения
        # модели больше не показываются вовсе (ai_agent._PROVENANCE_COLUMNS),
        # но она может знать о них из общей эрудиции — поэтому правило про
        # формулировки остаётся.
        ...
        f"Просто назови цифры и факты, которые пришли из базы.\n"
        f"6. НЕ отправляй пользователя никуда за ответом. Ты отвечаешь ПО "
        f"БАЗЕ ДАННЫХ УНИВЕРСИТЕТА и только по ней. Запрещены ссылки, адреса "
        f"страниц и любые отсылки: «подробнее на сайте», «уточняйте в "
        f"приёмной комиссии», «уточнить можно там-то», «полная информация по "
        f"ссылке», «обратитесь в деканат». Не называй сайты, документы и "
        f"внешние источники — их у тебя нет. Ответ должен быть "
