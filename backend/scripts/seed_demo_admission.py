...
Когда появятся официальные цифры, их вписывают в базу поверх этих строк по
естественному ключу (ключи объявлены в 006_official_reference.sql) — удалять
ничего не нужно, достаточно сменить data_status на 'official' и проставить
source_url.
...
def main(argv: list[str]) -> int:
    ...
    with _common.connect() as conn:
        with conn.cursor() as cur:
            ...
            if not programs:
                print("Справочник edu_programs пуст — заполните его в базе, "
                      "демо-места вешать не на что")
                return 1
