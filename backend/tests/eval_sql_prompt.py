...
ROUTING_CASES = [
    ...
    # --- успеваемость: только обезличенно, ФИО в схеме больше нет
    ("какой средний балл на факультете информационных технологий",
     {"student_rankings"}),
    ("сколько должников на кафедре программной инженерии",
     {"department_debts", "academic_debts"}),
    ("сколько заявлений подавали на экономику в прошлые годы",
     {"admission_dynamics"}),
    # --- студент: свои данные
    # Почта самого студента лежит в my_profile под именем student_email
    # (не email — иначе запрос отклоняет _assert_no_forbidden_columns).
    ("какая у меня почта в системе", {"my_profile"}),
    # --- студент: расписание
