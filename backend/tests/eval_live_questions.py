...
QUESTIONS = [
    ...
    # ФИО студентов не выводятся ни одной роли (миграция 014). Вопросы
    # переформулированы так, как их ставит регламент: сколько, какая доля,
    # какой средний — эти ответы схема даёт, а поимённые списки не даёт.
    ("Какой средний балл на факультете информационных технологий "
     "за последнюю сессию?", "deans-office", False),
    ("Сколько студентов группы ФИТ-0925-1 имеют академические "
     "задолженности?", "deans-office", False),
    ('Сколько должников учится на кафедре "Программная инженерия"?',
     "deans-office", False),
    ("Сколько студентов не сдали ни одного экзамена?", "deans-office", False),
    ...
    # ── абитуриент: своя роль, вне учебного контура ───────────────────
    ("Какие направления подготовки есть и сколько на них бюджетных мест?",
     "applicant", False),
    ("Какой был проходной балл на юриспруденцию в прошлом году?",
     "applicant", False),
    ("Сколько заявлений подавали на экономику в прошлые годы?",
     "applicant", False),
    ("До какого числа принимаются документы?", "applicant", False),

    # ── должны быть отклонены ─────────────────────────────────────────
    # Абитуриент ещё не учится: расписания и оценок о нём не существует.
    ("Покажи моё расписание на завтра.", "applicant", True),
    ("Какие у меня оценки за прошлый семестр?", "applicant", True),
